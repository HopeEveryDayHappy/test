...
def YCrCb2RGB(input_im):
    ...
    bias = torch.tensor([0.0 / 255, -0.5, -0.5]).cpu()
    temp = (im_flat + bias).mm(mat).cpu()
    out = (
        temp.reshape(
            list(input_im.size())[0],
            list(input_im.size())[2],
...
def train_fusion(num=0, logger=None):
    ...
    for epo in range(0, epoch):
        ...
        for it, (image_vis, image_ir, label, name) in enumerate(train_loader):
            fusionmodel.train()
            image_vis = Variable(image_vis).cpu()
            image_vis_ycrcb = RGB2YCrCb(image_vis)
            image_ir = Variable(image_ir).cpu()
...
def run_fusion(type='train'):
    ...
    with torch.no_grad():
        for it, (images_vis, images_ir, labels, name) in enumerate(test_loader):
            ...
            if args.gpu >= 0:
                images_vis = images_vis.cpu(args.gpu)
                images_ir = images_ir.cpu(args.gpu)
                labels = labels.cpu(args.gpu)
